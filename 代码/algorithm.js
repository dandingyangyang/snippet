...
/** 4. 归并排序: 递归分成左右两个数组，分到最后每个数组只剩一个数字的时候，再把左右两个数组当做有序数组合并起来。
 * 稳定性： 稳定
 * @param {*} arr 
 */
...
let quick = (arr, left, right, k) => {
    ...
    // 当left === right 时，返回arr[left]
    return arr[left]
}

let partition = (arr, left, right) => {
    // 取中间项为基准。返回的是一个基准值的位置（可能数组中有多个基准值，这里返最后一个基准值的位置）
...
function depulicate5(arr) {
    ...
    // 这个数组也可以不需要，使用Object.keys(obj)返回就行
    return otherArr;
}
/**
...
/**
 * 总结一些用法：详见https://github.com/CyC2018/CS-Notes/blob/master/notes/Leetcode%20%E9%A2%98%E8%A7%A3%20-%20%E4%BD%8D%E8%BF%90%E7%AE%97.md
 * 
 * 正数的原码、反码、补码都是它本身
 * 负数的原码是它本身
 * 负数的反码是：符号位不变，其余各个位取反
 * 负数的补码是：符号位不变，其余各个位取反再+1
 * 
 * 计算机中存储的都是补码（这样加法和减法都可以通过加法实现）
 * 
 * 0在内存的补码表示全部是0：00000000
 * -1在内存的补码表示全部都是1：11111111
 * 
 * 关于&运算
 * ① num & 1 ：得到num最后一位是否是1（可以用来判断奇数偶数）
 * ② num & (num - 1)：去掉 num 的二进制表示中最低的那一位 1
 * ③ num & (-num)：得到 num 的二进制表示中最低的那一位 1对应的数值
 * 
 * 
 * 关于 ^ 运算
 * ① x ^ x ^ y = y ：可以用来忽略成对的x, 找到单独的那个y
 * 
 * 右移
 * 3 >> 1 是1
 * 2 >> 1 是1
 * 
 * 
 * a.toString(2) 十进制的a转换成2进制
 * parseInt(b, 16) 16进制的b转换成十进制
 */


// 有时候二进制的问题也可以用正则做: (i).toString(2).match(/1/g)
// 338. 比特位计数

var countBits = function(n) {
    let result = [];
    for (let i = 0; i <= n; i++) {
        const reg = (i).toString(2).match(/1/g);
        result[i] = reg && reg.length ? reg.length : 0;
    }
    return result;
};
...
// 【注意】: 获取一个数字二进制表示中1的个数： 
// num & 1 得到最后一位是否是1, num = num >> 1 右移一位
// 或者：num = num & (num - 1) 去掉 n 的位级表示中最低的那一位 1，
...
//【注意】: x ^ x ^ y = y 可以忽略成对的x, 找到单独的那个y

var singleNumber = function(nums) {
    return nums.reduce((pre, current) => pre ^ current);
};

/**
 * 268. 丢失的数字: https://leetcode-cn.com/problems/missing-number/
 * 和上面一样，还是利用x ^ x ^ y = y 可以忽略成对的x, 找到单独的那个y
 */

var missingNumber = function(nums) {
    const data = nums.reduce((pre, current, index) => {
        return pre ^ index ^ current
    }, 0)
...
var letterCombinations = function(digits) {
    ...
    for (let i = 0; i < digits.length; i++) {
        let currentNodes = map[digits[i]];
        if (i === 0) {
            result = currentNodes.slice(0)
        } else {
            let preLen = result.length;
            while (preLen--) {
                const pre = result.shift();
...
var restoreIpAddresses = function(s) {
    ...
    function dfs(str, preArr) {
        if (preArr.length === 4) {
            if (!str) {
                result.push(preArr.join('.'));
            }
            return;
        }
        if (str[0] === '0') {
            let arr = preArr.slice(0);
            arr.push('0');
            return dfs(str.slice(1), arr)
        }
        for (let i = 0; i < str.length && i < 3; i++) {
            const currentStr = str.slice(0, i + 1);
            if (Number(currentStr) <= 255) {
                let newArr = preArr.slice(0);
                newArr.push(currentStr);
                dfs(str.slice(i + 1), newArr);
            }
        }
    }
...
var exist = function(board, word) {
    ...
    function dfs(board, visited, i, j, word) {
        if (!word) {
            result = true;
            return;
        }
        const row = board.length;
        const col = board[0].length;
        if (i < 0 || j < 0 || i >= row || j >= col || visited.find((item) => item[0] === i && item[1] === j) || board[i][j] !== word[0]) return;
        visited.push([i, j]);
        for (let d of directions) {
            const newX = d[0] + i;
            const newY = d[1] + j;
            dfs(board, visited.slice(0), newX, newY, word.slice(1))
        }
    }
...
// 46. 全排列 https://leetcode-cn.com/problems/permutations/solution/chou-xiang-cheng-jue-ce-shu-yi-ge-pai-lie-jiu-xian/
var permute = function(nums) {
    let result = [];
    let path = [];
    let used = [];

    function dfs(nums, count) {
...
var _rob = function(nums, start, end) {
    ...
    var pre1 = nums[start],
        pre2 = Math.max(nums[start], nums[start + 1]);
    var current = pre2;
...
var uniquePaths = function(m, n) {
    ...
    // 【注意】：new Array(3).map(() => 1) 这是map不会执行，因为new Array(3)里面是个empty数组，map不会执行，但是fill会执行
...
// 注意物品在外面层循环，包的承重在内层循环：第一个物品先去填充承重为1 2 3 4 5... 的包，得到结果，然后第一个物品和第二个物品一起去填充承重为 1 2 3... 的包，这时候可以利用第一次填充的结果了
...
// 可以优化成一维dp
/**
 *  dp[0] = 0; // 记得初始化dp[0], 这个很重要
 * for(let i = 0; i < n; i++) {
 *      //  注意这里的倒序：因为要访问dp[i-1][j-weights],所以不能从前面开始，否则会被覆盖，需要从后面开始
        for(let j = W; j > 0; j--) {
...
//  将一维dp方法中强调的倒序变成顺序
// 具体解释看上面的 代码参考地址  https://segmentfault.com/a/1190000012829866


/**
 * 关于物品和背包承重谁放在外层循环：
 * 1. 一般都是物品放在外层循环
 * 2. 求解顺序的完全背包问题时，对物品的迭代应该放在最里层，对背包的迭代放在外层，只有这样才能让物品按一定顺序放入背包中。
 * 如 139.字符串按单词列表分割
 */
//